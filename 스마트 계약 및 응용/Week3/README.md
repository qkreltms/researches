...
- OP_RETURN을 활용한 여러 서비스
  - proof of existence
    - 블록체인에 문서나 파일의 hash 값을 저장하게 해준다.
    - 파일의 내용이 공개되지 않으면서 자신이 이 파일을 소지했다는 것을 증명
  - colored coin(coinprism)
    - 파일 뿐만아니라 유형(집, 금), 무형(게임 아이템) 같은 자산을 블록체인에 담을 수 있다.
    - 이를 통해서 소유권 주장 가능
    - OP_RETURN 데이터에는 Open Assets을 의미하는 데이터, 프로토콜의 버전 정보, 자산 정보의 링크가 담기게 된다.
    - ![1](./70.png)
- Mastercoin
  - 비트코인을 하부 구조로하고 그 위에 새로운 코인을 개발할 수 있는 구조
  - ![1](./71.png)
  - OP_RETURN이 없었던 시기에 개발 UTXO의 output에 encoding 정보를 실어서 새로운 통화를 발행한다든지, 코인 간의 교환이나 게임에 코인을 거는 등의 다양한 기능을 추가할 수 있게 했다.
  - 추후에 OP_RETURN이 나온후에 OP_RETURN을 사용했다.
- 위의 서비스들은 이더리움이 나오면서 운영되지 않았다.
